...
def get_audio_with_effects(wav_path, augment=False):
    """
    Load audio and apply SoX effects if needed.
    """
    wav, fs = torchaudio.load(wav_path)  # Load audio

    effects = []  # Default: no effects

    if augment:
        # Speed (random between 0.9x and 1.1x)
        min_speed = 0.9
        max_speed = 1.1
        speed = np.random.uniform(min_speed, max_speed)
        effects.append(["tempo", str(speed)])

        # Volume adjustment (-10dB to +10dB)
        min_gain = -10
        max_gain = 10
        gain_dB = np.random.uniform(min_gain, max_gain)
        gain = 10**(gain_dB / 20)
        effects.append(["vol", str(gain)])

    if effects:
        wav, fs = torchaudio.sox_effects.apply_effects_tensor(wav, fs, effects)

    return wav[0].numpy(), fs
...
class SLUDataset(torch.utils.data.Dataset):
	...
	def __getitem__(self, idx):
		...
		#true_idx = idx
		idx = idx % len(self.df)  # Ensure index wraps around

		wav_path = os.path.join(self.base_path, self.df.loc[idx].path)

		# Load and apply effects
		x, fs = get_audio_with_effects(wav_path, augment=self.augment)

		# Apply additional augmentations (cropping, noise) if enabled
		if self.augment:
			# Crop (random length 90%-110% of original size)
			min_length = round(len(x) * 0.9)
			max_length = round(len(x) * 1.1)
			...
			if len(x) > length:
				...
				x = np.pad(x, (0, length - len(x)), mode="constant")

			# Add noise with random SNR
			snr = np.random.choice(self.SNRs, p=[1 / len(self.SNRs)] * len(self.SNRs))
